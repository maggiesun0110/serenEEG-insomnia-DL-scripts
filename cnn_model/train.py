...
# -------------------------------
# 5. Training with TensorBoard & Early Stopping
# -------------------------------
def train_model_full(model, train_loader, test_loader, test_subject_ids, y_train, device, epochs=50, lr=1e-3, patience=5, log_dir="./runs/eeg_experiment"):
    ...
    val_accuracies, val_f1s, val_precisions, val_recalls, val_roc_aucs = [], [], [], [], []
    per_subject_acc_history = {subj: [] for subj in np.unique(test_subject_ids)}
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            for X_batch, y_batch in test_loader:
                ...
                all_probs.append(probs[:,1].cpu().numpy())  # binary ROC-AUC
        ...
        val_loss = criterion(torch.tensor(all_probs).to(device), torch.tensor(all_labels).to(device)).item()
        ...
        for subj in per_subject_acc_history:
            mask = test_subject_ids == subj
            subj_acc = (all_preds[mask] == all_labels[mask]).mean()
            per_subject_acc_history[subj].append(subj_acc)
            writer.add_scalar(f"Accuracy/Subject_{subj}", subj_acc, epoch)
        
        print(f"Epoch {epoch+1}/{epochs}: Train Loss={avg_train_loss:.4f}, Val Loss={val_loss:.4f}, Acc={acc:.4f}, F1={f1:.4f}, ROC-AUC={roc_auc:.4f}")
...
if __name__ == "__main__":
    ...
    train_model_full(model, train_loader, test_loader, test_subject_ids, y_train, device, epochs=50, lr=1e-3, patience=7)
    
    print("Run `tensorboard --logdir=runs` to visualize metrics and curves!")
